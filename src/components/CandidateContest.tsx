...
const ChoicesGrid = styled.div<{ isScrollable: boolean }>`
  ...
  @media (min-width: ${tabletMinWidth}px) {
    padding-right: ${({ isScrollable }) => (isScrollable ? '8rem' : '1rem')};
    padding-left: 1rem;
  }
`
...
class CandidateContest extends React.Component<Props, State> {
  ...
  public updateContestChoicesScrollStates = () => {
    ...
    this.setState({
      ...
      isScrollable:
        isTabletMinWidth && target.scrollHeight > target.clientHeight,
    })
  }

  public scrollContestChoices = (event: ButtonEvent) => {
